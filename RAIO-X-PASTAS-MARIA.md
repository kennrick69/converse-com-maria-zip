...
| `converse-com-maria-site` | 20 KB | `converse-com-maria-site` | 2026-01-13 | 🟢 **REPO DA PÁGINA BETA DE TESTERS (no Hostinger)** — MANTÉM, está em uso |

**Recomendação direta** (corrigida após esclarecimento do JOs):
- **MANTER `proj_maria/`** — única pasta de código do app (frontend + backend + landing-page oficial).
- **MANTER `converse-com-maria-site/`** — fonte da página BETA pra convocar testers no Hostinger (`conversecommaria.com.br`). Está em uso.
...
**Risco de apagar**: **ALTO** ⚠️
- **Esclarecimento do JOs (2026-05-25)**: o site no Hostinger (`conversecommaria.com.br`) é a **página BETA pra convocar testers**, e o `proj_maria/landing-page/` é o **site oficial que vai pro ar futuramente**. **Essas são duas páginas DIFERENTES com propósitos diferentes**, não versões da mesma coisa.
- O `converse-com-maria-site/` é o repo fonte da **página de testers no Hostinger** — ainda VIVO e útil enquanto a fase beta continuar.
- **NÃO APAGAR** essa pasta. **MANTER** como repo da página de testers até a fase beta terminar.
- Quando o site oficial (`proj_maria/landing-page/`) entrar no ar, aí sim o repo `converse-com-maria-site/` pode ser arquivado.
...
### 🟢 `converse-com-maria-site/` (página BETA testers) e `proj_maria/landing-page/` (site oficial) — DIFERENTES, AMBAS VIVAS
**Corrigido após esclarecimento do JOs (2026-05-25)**: são duas páginas com propósitos distintos.
- **`conversecommaria.com.br` (Hostinger)** = página **BETA pra convocar testers** durante a fase de testes na Play Store. Fonte: repo `converse-com-maria-site/` (este). MANTER.
- **`proj_maria/landing-page/`** = site **OFICIAL** que vai pro ar no futuro (quando sair do beta). Não tem deploy ainda.
- Site beta no Hostinger não foi atualizado desde 2026-01-13 — provavelmente está estável e não precisa mexer durante a fase de testes.
- Quando o app sair do beta e o site oficial entrar no ar, aí sim o repo de testers pode ser arquivado.
...
## 5. Recomendação final em 2 passos

1. **Mantém** estas pastas:
   - `C:\Projetos\proj_maria\` — única pasta de código do app
   - `C:\Projetos\converse-com-maria-site\` — repo da página BETA de testers (Hostinger)
2. **Apaga** essas 2 locais (zero perda — repos GitHub continuam intactos):
   - `C:\Projetos\converse-com-maria\` (repo paralelo abandonado)
   - `C:\Projetos\converse-com-maria-zip\` (duplicata, WIP = só CRLF noise)

Comandos quando você aprovar (eu não executo agora):
```bash
# Backup paranoia (opcional, só os .git que somam 5 MB)
...
# Apagar locais (Windows mantém ambos repos no GitHub)
