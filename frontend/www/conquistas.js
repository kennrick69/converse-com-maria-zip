...
const SistemaConquistas = {
    // Helper para renderizar ícone (SVG ou emoji fallback)
    renderIcone: function(conquista, tamanho = 'normal') {
        const tamanhos = {
            'small': 'w-8 h-8',
            'normal': 'w-16 h-16',
            'large': 'w-24 h-24',
        ...
        const classe = tamanhos[tamanho] || tamanhos['normal'];
        
        // Se tem SVG, usa imagem
        if (conquista.svg) {
            return `<img src="conquistas-svg/${conquista.svg}.svg" alt="${conquista.nome}" class="${classe} drop-shadow-lg" />`;
        }
        // Fallback para emoji
        const fontSizes = {
            'small': 'text-2xl',
        ...
        return `<span class="${fontSizes[tamanho] || 'text-4xl'}">${conquista.icone}</span>`;
    },
    ...
    salvarConquistadas(lista) {
        ...
        if (window.UserDataService && window.FirebaseService?.isLoggedIn()) {
            lista.forEach(async (conquistaId) => {
                try {
                    await UserDataService.addConquista(conquistaId);
                } catch (e) {
                    // Ignora erros de duplicata
                }
            });
            console.log('🔥 Conquistas sincronizadas com Firebase:', lista.length);
        }
    },
    ...
    mostrarNovaConquista(conquista, onClose) {
            ...
            <div class="text-center animate-bounce-in">
                ...
                <div class="relative mb-6">
                    <div class="w-32 h-32 mx-auto bg-gradient-to-br from-yellow-400 via-yellow-500 to-orange-500 rounded-full flex items-center justify-center shadow-2xl medalha-conquistada">
                        ${SistemaConquistas.renderIcone(conquista, 'xlarge')}
                    </div>
        ...
        // 📳 VIBRAR CELULAR - Capacitor Haptics (mais confiável)
        try {
            if (window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.Haptics) {
                // Usar Capacitor Haptics - padrão de sucesso
                window.Capacitor.Plugins.Haptics.notification({ type: 'SUCCESS' });
                console.log('📳 Haptics: notification SUCCESS');
            } else if (navigator.vibrate) {
                // Fallback para navigator.vibrate
                navigator.vibrate([100, 50, 100, 50, 200]);
                console.log('📳 Vibração via navigator.vibrate');
            }
        } catch(e) {
            console.log('Vibração não disponível:', e);
        }
        
        // 🔊 SOM DE CONQUISTA (AudioContext para mobile)
        try {
            const audioCtx = new (window.AudioContext || window.webkitAudioContext)();
            
            // Criar um "ding" de conquista
            ...
            // Som agradável (nota musical ascendente)
            oscillator.frequency.setValueAtTime(880, audioCtx.currentTime); // Lá
            oscillator.frequency.setValueAtTime(1108.73, audioCtx.currentTime + 0.1); // Dó#
            oscillator.frequency.setValueAtTime(1318.51, audioCtx.currentTime + 0.2); // Mi
            ...
            console.log('🔊 Som de conquista tocado!');
        } catch(e) {
            console.log('AudioContext erro:', e);
        }
    },
                            ...
                            <div class="flex justify-center gap-2 mb-3">
                                ${ultimasConquistas.length > 0 
                                    ? ultimasConquistas.map(c => `
                                        <div class="w-14 h-14 bg-gradient-to-br from-yellow-400 to-orange-500 rounded-full flex items-center justify-center shadow-lg p-1">
                                            ${SistemaConquistas.renderIcone(c, 'small')}
                                        </div>
                                    `).join('')
                                        ...
                                        return `
                                            <div class="relative cursor-pointer ${obtida ? '' : 'opacity-40'}" onclick="SistemaConquistas.mostrarSignificado('${c.id}')">
                                                <div class="bg-white/5 rounded-xl p-3 border ${obtida ? 'border-yellow-500/50' : 'border-white/10'} text-center hover:bg-white/10 transition-all">
                                                    <div class="w-12 h-12 mx-auto mb-2 rounded-full ${obtida ? 'bg-gradient-to-br from-yellow-400 to-orange-500' : 'bg-white/10'} flex items-center justify-center p-1 ${obtida ? '' : 'grayscale'}">
                                                        ${SistemaConquistas.renderIcone(c, 'small')}
                                                    </div>
        ...
        modal.innerHTML = `
            <div class="bg-gradient-to-br from-purple-900 to-indigo-900 rounded-3xl p-6 max-w-sm w-full shadow-2xl border border-purple-500/30 animate-scale-in">
                <div class="text-center mb-4">
                    <div class="w-20 h-20 mx-auto mb-3 rounded-full ${obtida ? 'bg-gradient-to-br from-yellow-400 to-orange-500' : 'bg-white/10'} flex items-center justify-center shadow-lg p-1 ${obtida ? '' : 'grayscale'}">
                        ${SistemaConquistas.renderIcone(conquista, 'large')}
                    </div>
    ...
    async gerarCanvasHTML() {
        ...
        const medalhasHTML = ultimasConquistas.length > 0 
            ? ultimasConquistas.map(c => `
                <div style="width: 64px; height: 64px; background: linear-gradient(135deg, #facc15, #f97316); border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 6px rgba(0,0,0,0.3); padding: 8px;">
                    ${c.svg 
                        ? `<img src="conquistas-svg/${c.svg}.svg" alt="${c.nome}" style="width: 100%; height: 100%;" />`
