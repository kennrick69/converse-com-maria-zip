...
const MuralIntencoes = {
    ...
    async carregarIntencoesFirebase() {
        if (typeof firebase === 'undefined' || !firebase.firestore) {
            ...
            // Se não tem Firebase, usa apenas as fixas
            this.intencoesComunidade = [...this.intencoesFake];
            return;
        }
        ...
        try {
            ...
            // ✅ MESCLAR: Intenções do Firebase + Fixas
            const intencoesFirebase = [];
            snapshot.forEach(doc => {
                const data = doc.data();
                // Filtrar visíveis no cliente
                if (data.visivel !== false) {
                    intencoesFirebase.push({ id: doc.id, ...data });
                }
            });
            
            // Mesclar: Firebase primeiro (mais recentes), depois as fixas
            this.intencoesComunidade = [...intencoesFirebase, ...this.intencoesFake];
            
            console.log(`🙏 ${intencoesFirebase.length} intenções do Firebase + ${this.intencoesFake.length} fixas = ${this.intencoesComunidade.length} total`);
        } catch (error) {
            console.error('Erro ao carregar intenções:', error);
            // Em caso de erro, usa apenas as fixas
            this.intencoesComunidade = [...this.intencoesFake];
        }
    },
    ...
    escutarIntencoes() {
        ...
        this.unsubscribe = db.collection('intencoes')
            ...
            .onSnapshot(snapshot => {
                ...
                // Mesclar: Firebase primeiro, depois as fixas
                this.intencoesComunidade = [...intencoesFirebase, ...this.intencoesFake];
