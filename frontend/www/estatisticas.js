...
const EstatisticasOracao = {
    ...
    // ========================================
    // 🔥 HELPER: SINCRONIZAR COM FIREBASE
    // ========================================
    async sincronizarFirebase(campo, valor, operacao = 'increment') {
        // Verificar se Firebase está disponível e usuário logado
        if (window.UserDataService && window.FirebaseService?.isLoggedIn()) {
            try {
                if (operacao === 'increment') {
                    await UserDataService.incrementStat(campo, valor || 1);
                } else if (operacao === 'set') {
                    await UserDataService.updateStats(campo, valor);
                }
                console.log(`🔥 Firebase: ${campo} ${operacao === 'increment' ? '+' + (valor || 1) : '= ' + valor}`);
            } catch (error) {
    ...
    registrarTerco(misterio) {
        ...
        this.sincronizarFirebase('tercosCompletos', 1);
        this.sincronizarFirebase('aveMariasRezadas', 53);
        this.sincronizarFirebase('paiNossosRezados', 6);
        this.sincronizarFirebase('minutosEmOracao', 20);
        
        // 🏆 VERIFICAR CONQUISTAS após registrar
        if (window.SistemaConquistas) {
            setTimeout(() => SistemaConquistas.verificarEMostrar(), 500);
        }
    },
    ...
    registrarTercoIniciado() {
        ...
        // 🔥 Sincronizar com Firebase
        this.sincronizarFirebase('tercosIniciados', 1);
    },
    ...
    registrarVela(tipo) {
        ...
        // 🔥 Sincronizar com Firebase
        this.sincronizarFirebase('velasAcesas', 1);
    ...
    registrarIntencao() {
        ...
        // 🔥 Sincronizar com Firebase
        this.sincronizarFirebase('intencoesPublicadas', 1);
    ...
    registrarOracaoPorIntencao() {
        ...
        // 🔥 Sincronizar com Firebase
        this.sincronizarFirebase('intencoesRezadas', 1);
    ...
    registrarMensagem() {
        ...
        // 🔥 Sincronizar com Firebase
        this.sincronizarFirebase('mensagensEnviadas', 1);
        this.sincronizarFirebase('minutosEmOracao', 2);
        
        // 🏆 VERIFICAR CONQUISTAS após registrar (só a cada 5 mensagens para não sobrecarregar)
        if (window.SistemaConquistas && dados.mensagensEnviadas % 5 === 0) {
            setTimeout(() => SistemaConquistas.verificarEMostrar(), 500);
        }
    },
    ...
    registrarVersiculo() {
        ...
        // 🔥 Sincronizar com Firebase
        this.sincronizarFirebase('versiculosLidos', 1);
    },
    ...
    atualizarStreak(streakAtual) {
        ...
        // 🔥 Sincronizar com Firebase
        if (window.UserDataService && window.FirebaseService?.isLoggedIn()) {
            UserDataService.updateStreak(streakAtual);
        }
    },
    ...
    registrarNovena(completa = false) {
        ...
        // 🔥 Sincronizar com Firebase
        this.sincronizarFirebase(completa ? 'novenasCompletas' : 'novenasIniciadas', 1);
